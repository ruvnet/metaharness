#!/usr/bin/env node
// @metaharness/example-huggingface — one-command scaffold.
// Delegates host wiring to the metaharness CLI, then prints huggingface-specific
// next steps. Read-only / sandbox by default; mutations need --allow-mutations.
import { execSync } from 'node:child_process';
import process from 'node:process';

const PKG = '@metaharness/example-huggingface';
const SDK = "@huggingface/inference @huggingface/hub";
const ENV_VARS = ["HF_TOKEN"];
const COMMAND = "/hf-discover";
