#!/usr/bin/env node
// @metaharness/example-stripe — one-command scaffold.
// Delegates host wiring to the metaharness CLI, then prints stripe-specific
// next steps. Read-only / sandbox by default; mutations need --allow-mutations.
import { execSync } from 'node:child_process';
import process from 'node:process';

const PKG = '@metaharness/example-stripe';
const SDK = "stripe";
const ENV_VARS = ["STRIPE_API_KEY (use sk_test_\u2026 for test mode)"];
const COMMAND = "/stripe-billing";
