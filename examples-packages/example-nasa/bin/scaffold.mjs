#!/usr/bin/env node
// @metaharness/example-nasa — one-command scaffold.
// Delegates host wiring to the metaharness CLI, then prints nasa-specific
// next steps. Read-only / sandbox by default; mutations need --allow-mutations.
import { execSync } from 'node:child_process';
import process from 'node:process';

const PKG = '@metaharness/example-nasa';
const SDK = "satellite.js";
const ENV_VARS = ["NASA_API_KEY (DEMO_KEY works for low rate)"];
const COMMAND = "/nasa-passes";
