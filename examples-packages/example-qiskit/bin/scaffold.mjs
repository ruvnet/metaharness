#!/usr/bin/env node
// @metaharness/example-qiskit — one-command scaffold.
// Delegates host wiring to the metaharness CLI, then prints qiskit-specific
// next steps. Read-only / sandbox by default; mutations need --allow-mutations.
import { execSync } from 'node:child_process';
import process from 'node:process';

const PKG = '@metaharness/example-qiskit';
const SDK = "(IBM Qiskit Runtime REST \u2014 no npm SDK; uses fetch)";
const ENV_VARS = ["QISKIT_IBM_TOKEN"];
const COMMAND = "/quantum-sim";
