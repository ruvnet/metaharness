#!/usr/bin/env node
// @metaharness/example-bio — one-command scaffold.
// Delegates host wiring to the metaharness CLI, then prints bio-specific
// next steps. Read-only / sandbox by default; mutations need --allow-mutations.
import { execSync } from 'node:child_process';
import process from 'node:process';

const PKG = '@metaharness/example-bio';
const SDK = "(NCBI E-utilities / Ensembl REST \u2014 no npm SDK; uses fetch)";
const ENV_VARS = ["NCBI_API_KEY (optional, raises rate limit)"];
const COMMAND = "/bio-lookup";
