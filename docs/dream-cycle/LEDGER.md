...
| 2026-08-24 | host-adapters | host-rvm's generated `install-rvm.sh` had a live RCE: `spec.name` unescaped in 2 double-quoted bash sites (`$(...)`/backtick command substitution) AND 2 raw `#`-comment header lines (bare newline breaks out, no quoting needed); round-1 critic caught the comment-line sibling the author missed, round-2 critic independently confirmed closed | #223 | #224 | yes | ACCEPT-WITH-CAVEATS | host-rvm 41/41 (0 regressions), dependent bench 5/5, tsc+build clean; real bash-exec repro both rounds (marker file created pre-fix, not post-fix) | `fc5f47bc...` | #200/#205/#207/#210/#212 still OPEN as of 2026-08-24; **#224 MERGED 2026-09-01** (confirmed by this row's own author — see 2026-09-01 row below) |
| 2026-08-25..08-31 | (various) | **Ledger gap, not a pipeline gap**: 7 more Dream Cycle nights ran in this window (branches `dream/2026-08-25-*` through `dream/2026-08-31-*`, PRs #228–#255: #229, #231, #240, #243, #246, #249, #255) but never got a ledger row on `main`, because every one of those PRs remains unmerged as of 2026-09-01. Verified directly via `git ls-remote`+`search_pull_requests`, not inferred. See issue #257 for the full count and citations. | n/a | n/a | n/a | n/a | n/a (see individual PR bodies on their branches for each night's receipt) | n/a | 7 PRs, 0 merged as of 2026-09-01 |
| 2026-09-01 | flywheel-promotion | Hand-merged 3 semantically-compatible but git-conflicting unmerged replay-trust fixes (#205, #231, #255 — all touching the same `gateReExecutes` region) into one clean diff on `verifyReplayBundle`; independent critic then found a 4th, undisclosed gap (`parents`/`generation` unbound in `sealedFieldsAuthentic`, letting 2 genuinely-signed commits be spliced into a fabricated chain) — closed in the same diff, not spun into a 5th PR. **Mid-review-cycle update**: while this PR sat open, #224 (host-rvm RCE fix, flagged in issue #257 as top backlog priority) was merged to `main` — direct evidence the backlog triage recommendation was acted on. A second same-day merge (#260, `feat(darwin-mode): add numeric genome kind`) independently claimed ADR-272; this PR's ADR was renumbered ADR-273 to resolve the collision, and this branch was merged with `origin/main` (merge commit, not rebase) to pick up both changes before re-push. | #257 | #258 | yes | ACCEPT-WITH-CAVEATS | 72/72 flywheel tests (was 71/71 pre-4th-gap-fix), 18/41 new-test assertions non-vacuously proven vs. unpatched main, all 7 real committed ReplayBundles unchanged PASS, 716/716 non-skipped tests across flywheel+evals-math+darwin-mode | `f49c9f51...` | #180/#181 MERGED; #187/#188 MERGED; #199/#200 MERGED; #223/#224 MERGED (2026-09-01, mid-cycle); #260 MERGED (2026-09-01, mid-cycle, unrelated darwin-mode work); #204/#205, #206/#207, #209/#210, #211/#212 still OPEN; #228-255 (7 PRs, 2026-08-25..08-31) OPEN/UNMERGED — this PR supersedes #205/#231/#255 in effect |
