...
| 2026-08-27 | (process — see #239) | Ledger silently broken 8 nights (STEP 25 wasn't reaching `main`) + review backlog escalation; backfilled #200-#231 locally and flagged the review queue rather than generating a 9th unreviewed candidate. This process fix (PR #240) itself sat unmerged; #199-#212's real backfilled rows above landed via #246 instead (2026-08-29), which is why this row has no separate backfill of its own. | #239 | #240 (unmerged as of 2026-08-31) | n/a | n/a | n/a | n/a | #200/#205/#207/#210/#212 still OPEN at the time |
...
| 2026-08-31 | flywheel-promotion | `verifyReplayBundle`'s `gateReExecutes` (ADR-235) re-gated on sealed `baselineScore`/`candidateScore`/`anchorScore` that lived outside the signed receipt payload; fixed by binding them (+ the receipt's own `id`/`target`/`verdict`/`primaryDelta`) into the signature. Independent critic found the first version insufficient (receipt-cloning bypass via unbound `id`) — 2nd commit closed it, test reproduces the critic's exact scenario. Dominant finding at PR-creation time was process, not code: backlog escalated in #239 (12 unmerged `dream/*` PRs, 2 disclosed RCE fixes, 16 days) — reported via issue comment + direct operator notification instead of a 13th redundant PR. **Update on merge (this row, same night):** #224 and #246 (both disclosed RCE fixes) merged to `main` shortly after, alongside #200's generator-genome fix and #226 (avo integration) — the backlog is moving, not stalled; this PR rebased onto that progress. | #254 | #255 | yes | ACCEPT-WITH-CAVEATS | 51/51 flywheel tests pass (0→51 incl. 4 new; 3/3 new tests fail on baseline), 0 regressions, 2 real committed bundles re-verified pass:true unchanged | `72332c3b...` | #224/#246 (RCE fixes) + #200 MERGED since PR creation; #205/#207/#210/#212/#229/#231/#240/#243/#249 still OPEN |
