...
## Empirical plan (what we will measure, mirroring ADR-167's gates)

- **Discrimination on real Terraform** (analogue of `real-corpus-scan`): a labeled corpus of templates with *genuine* savings opportunities (gp2→gp3, over-provisioned RDS, no S3 lifecycle, idle NAT) plus *traps* (a "cheap" change that breaks the build or fails checkov). The oracle must accept the genuine savings and reject every trap. **Precision/recall on accepted patches.**
