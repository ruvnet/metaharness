...
- **Scaled multi-seed gate** (`frontier-multiseed-large.json`, **14 funcs × 3 seeds = 42 cells/model**, temp sweep 0.4→1.3, **5-model cost curve**): the larger, more diverse re-run that retires the "N is still modest" caveat. **Qwen3-235B 0.952 verified-rate @ 0.028 mUSD/verified** — top recall on the curve, ahead of qwen-2.5-72b (0.905), GLM-5.2 (0.857 @ **1.009 mUSD, ~36×**), deepseek-v3.2 (0.833), and the cheap floor qwen-7b (0.738). Paired bootstrap vs the GLM incumbent: **qwen3-235b−glm meanDelta +0.095, lower95 = 0, p≈0.06** — again **not statistically worse, higher mean, ~36× cheaper**, and the *only* challenger to reach lower95 ≥ 0. The Qwen3-235B default is confirmed on the larger corpus. **The entire gated experiment cost $0.0428** (budget cap $5.00, not hit) — empirical proof of the cost-efficiency thesis: at this token economy a human-scale budget cannot be "spent" on model selection without thousands of redundant calls past statistical convergence.
...
**What hurts.** The frontier-lane selection is now **multi-seed bootstrap-gated** at modest scale (42 cells/model, scaled run), but the *discovery/discrimination* scans remain mostly single non-deterministic runs and the corpora are small (toy fixtures + a few real packages + planted vulns); findings are robustness/CWE-class, **not novel zero-days**. The remaining honest gap is a **labeled real-CVE corpus at true scale** — that is *data*-gated (sourcing/labelling real CVEs), not budget-gated; throwing money at the existing benches only buys redundancy. This is **not Mythos-class** and makes no such claim.
...
- Receipts: `packages/projects/bench/results/{zero-day-discovery,real-corpus-scan,real-cve-corpus,proposer-bakeoff,chinese-frontier-bakeoff,frontier-multiseed,frontier-multiseed-large,learning-loop,learning-loop-large,escalation-llm,handoff-llm}.json`.
