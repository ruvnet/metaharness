...
// host adapter. The 11th host, per ADR-247.
//
...
//
// Verified integration surface (ADR-247, research doc §3):
...
//
// CRITICAL SECURITY NOTE (ADR-247 §2.2, fail-closed): Prime Agent executes
 ...
 * allow rules — the intended scope (ADR-247 §2.2: allow entries are
...
export function skillMd(tool: ToolSpec, spec: HarnessSpec, resolvedName?: string): string {
  ...
  const lines = [
    ...
    description,
    '',
    'This skill is generated by @metaharness/host-prime-agent (ADR-247). The',
...
export function mcpIntegrationPy(serverName: string, url: string): string {
  return [
    '# SPDX-License-Identifier: MIT',
    '# Auto-generated by @metaharness/host-prime-agent (ADR-247).',
...
/**
 * SANDBOX-REQUIRED.md — the fail-closed artifact (ADR-247 §2.2). Emitted
...
export function installMd(spec: HarnessSpec): string {
  ...
  // ignored (ADR-044 capability-coverage discipline / ADR-247 test contract 5).
  ...
  // ADR-246 §2.2 autonomous block. Only fields actually present are
  ...
  if (autonomous) {
    ...
    lines.push('## Autonomous invocation (ADR-246)', '', '```bash', cli.join(' '), '```', '');
