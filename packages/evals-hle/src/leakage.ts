// @metaharness/evals-hle — the LEAKAGE DETECTOR (fail-closed).
//
...
import type { HLEPolicyGenome } from './genome.js';
...
export interface LeakageCheck {
  ...
  /** Policy mentions a benchmark artifact ("HLE", "humanity's last exam", dataset ids, split names). */
...
const ARTIFACT_RE = /\b(hle|humanity'?s last exam|cais\/hle|frozen[- ]?holdout|private[- ]?validation|test split|gold answer)\b/i;
const HACK_RE = /\b(if (the )?question (contains|is|mentions)|answer\s*(is|=)\s*["']|lookup table|memoized answer|hardcod)/i;

/** Scan a candidate genome for leakage. `publicExamples` are dev-set questions the policy MUST NOT encode. */
export function detectLeakage(genome: HLEPolicyGenome, publicExamples: string[] = []): LeakageCheck {
...
function collectStrings(g: HLEPolicyGenome): string[] {
  const out: string[] = [];
