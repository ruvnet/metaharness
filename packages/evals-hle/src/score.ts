// @metaharness/evals-hle — the SCORE VECTOR and its projection onto the flywheel's four frozen axes.
//
// HLE is not optimized on raw accuracy. We measure a vector (accuracy, cost/correct, calibration error,
// format-error rate, per-subject accuracy) and PROJECT it onto the flywheel's opaque `Score`:
//   primary     = accuracy
//   noopRate    = format-error + abstention rate  ("commit more" — the frozen gate needs this to strictly ↓)
...
// The richer axes (calibrationError, perSubjectAccuracy, formatErrorRate) ride ALONG on `HleScore` so the
// composite HLE gate can enforce its extra clauses — the base `Score` fields alone drive the frozen gate.
import type { Score } from '@metaharness/flywheel';
import type { Subject } from './genome.js';

export interface HleScore extends Score {
  accuracy: number;
...
/** Aggregate per-question results into an HleScore (which IS a valid flywheel Score). */
export function projectScore(results: PerQuestionResult[]): HleScore {
