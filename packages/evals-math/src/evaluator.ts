// @metaharness/evals-math — the EVALUATOR (harness pipeline) + the PROPOSER (schema-constrained mutation).
//
// These are the two seams where the model/benchmark enters the flywheel. Everything math-specific lives HERE,
...
import { projectScore, type MathScore, type PerQuestionResult } from './score.js';
import type { MathItem } from './data.js';
...
export interface MathEvaluatorOpts {
  solve: SolveFn;
  judge?: Judge;
...
export function makeMathEvaluator(opts: MathEvaluatorOpts): Evaluator {
  ...
  return async function evaluate(policy: Policy, suite: Suite): Promise<MathScore> {
    const genome = policyToGenome(policy);
    const leaked = leaks(detectLeakage(genome, publicExamples));
    const items = suite.items as MathItem[];
...
export interface MathProposerOpts {
...
export function makeMathProposer(opts: MathProposerOpts = {}): Proposer {
    ...
    if (opts.complete) {
      ...
      try {
        suggestion = (await opts.complete(
          ...
          `You tune ONE lever of a math-solving policy. Lever="${target}", current="${cur}". Reply with ONLY the new value, in-schema. No prose.`,
        )).trim();
...
export function deterministicStep(target: string, cur: string): string {
  ...
  switch (target) {
    case 'solverStyle': return nextEnum(SOLVER_STYLES, cur || 'direct');
    case 'answerFormat': return nextEnum(ANSWER_FORMATS, cur || 'integer');
...
export function clampLever(target: string, proposed: string, current: string): string {
  const num = (v: string, d: number) => (Number.isNaN(Number(v)) ? d : Number(v));
  switch (target) {
    case 'solverStyle': return valid(SOLVER_STYLES, proposed) ?? nextEnum(SOLVER_STYLES, current || 'direct');
    case 'answerFormat': return valid(ANSWER_FORMATS, proposed) ?? nextEnum(ANSWER_FORMATS, current || 'integer');
