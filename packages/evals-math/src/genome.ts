// @metaharness/evals-math — the MATH POLICY GENOME.
//
// The genome is deliberately small, typed, and auditable. The flywheel evolves the HARNESS POLICY that
// decides how to answer/verify/route/abstain/calibrate — NOT the model. Models, grader, and the private
...
/** Answer shapes a math word-problem answer takes. Chosen per subject; drives the normalizer + verifier.
 *  All are numeric-family — GSM8K answers are numbers, so exact-match after numeric normalization is the
 *  primary metric. `expression` covers closed-forms; `units` covers answers that carry a physical unit. */
export const ANSWER_FORMATS = ['integer', 'decimal', 'fraction', 'expression', 'units'] as const;
 ...
 *  `symbolicRecompute` — re-derive/re-evaluate the arithmetic and compare — is the big recoverable bucket. */
export const VERIFICATION_MODES = ['none', 'symbolicRecompute', 'unitCheck', 'multiSolver', 'rangeSanity'] as const;
 ...
 *  `chain-of-thought` is the GSM8K workhorse; `verify-then-answer` folds the recompute step into the prompt. */
export const SOLVER_STYLES = ['direct', 'chain-of-thought', 'answer-first', 'verify-then-answer'] as const;
export type SolverStyle = (typeof SOLVER_STYLES)[number];

/** The math subtopics GSM8K-style problems span. The classifier maps a question to one of these; the
 *  verifier stack is chosen per subject. Kept fixed + small on purpose (a mutable taxonomy is a leakage
 *  surface). */
export const SUBJECTS = [
  'arithmetic', 'algebra', 'geometry', 'numbertheory', 'combinatorics', 'wordproblem', 'calculus', 'other',
] as const;
...
export interface MathPolicyGenome {
...
export function rootGenome(): MathPolicyGenome {
...
export function policyFor(genome: MathPolicyGenome, subject: Subject): SubjectPolicy {
...
export function genomeToPolicy(g: MathPolicyGenome): Record<string, string> {
  return {
...
export function policyToGenome(p: Record<string, string>): MathPolicyGenome {
