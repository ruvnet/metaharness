// @metaharness/evals-math — CONFIDENCE CALIBRATION.
//
...
// reliability gap): a well-calibrated policy's mean confidence on the answers it commits to should track its
// actual accuracy. Worsening calibration blocks promotion (folded into the math gate) — a policy that gets
// answers right but lies about its confidence is not an improvement.
import type { ConfidenceRule } from './genome.js';

export interface ConfidenceInput {
  /** Model self-reported logprob-ish confidence in [0,1], if available. */
  logprob?: number;
  /** Self-consistency agreement in [0,1] (fraction of samples agreeing). */
...
/** Expected Calibration Error over committed answers, binned. Lower is better. Abstentions are excluded
 *  (you cannot be mis-calibrated about an answer you declined to give). */
