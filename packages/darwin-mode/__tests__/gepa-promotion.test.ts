...
// Ported from bench/swebench/gepa/learn.test.mjs (assertions unchanged): the STRICT promotion
// predicate (gold-regress→reject; empty-patch-worse→reject; both-improve→promote), the report
// shape, and the composite key.
...
} from '../src/gepa/promotion.js';

// Fixture: a details map from an array of [gold, failureClass, thrash, cost] tuples.
const mkDetails = (rows: Array<[boolean, number, number?, number?]>) => Object.fromEntries(rows.map(([gold, fc, thrash, cost], i) =>
  [`inst-${i}`, { gold, failureClass: fc, thrash, cost }]));
const mkEval = (rows: Array<[boolean, number, number, number]>, extra: Record<string, unknown> = {}) => {
...
const SEED_HOLDOUT_ROWS: Array<[boolean, number]> = [ // [gold, failureClass]
  [false, 3], [false, 3], [false, 4], [false, 3], [true, 0], [false, 3],
  [true, 0], [false, 4], [false, 3], [false, 3], [false, 3], [false, 5],
];
const CAND_HOLDOUT_ROWS: Array<[boolean, number]> = [
  [false, 3], [false, 3], [false, 6], [false, 2], [true, 0], [false, 3],
  [true, 0], [false, 5], [true, 0], [false, 4], [false, 3], [false, 4],
];
const mkHoldout = (rows: Array<[boolean, number]>, perCost: number) => mkEval(rows.map(([g, fc]) => [g, fc, 0, perCost]));
...
test('STRICT rule — REJECT when gold regresses (a seed-resolved instance is lost)', () => {
  ...
  const lose = CAND_HOLDOUT_ROWS.map((r, i) => (i === 6 ? [false, 3] as [boolean, number] : r));
...
test('STRICT rule — REJECT when empty-patch rate does not improve', () => {
  ...
  const worseEmpty = SEED_HOLDOUT_ROWS.map(([g, fc]) => (fc === 4 || fc === 5 ? [g, 3] as [boolean, number] : [g, fc] as [boolean, number]));
