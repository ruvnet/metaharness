...
// ADR-228 §9.1 — ported from bench/swebench/gepa/gepa-loop.test.mjs (assertions unchanged): Pareto
// math, parent sampling, target selection from ASI votes, reflection parsing, and the end-to-end
// budgeted loop with scripted evaluator/reflector.
...
} from '../src/gepa/loop.js';
import { SEED_GENOME, type Genome } from '../src/gepa/genome.js';
...
test('gepaOptimize end-to-end: accepts improving mutation, Pareto-adds subset winner, respects budget', async () => {
  ...
  const evaluate = async (genome: Genome) => {
