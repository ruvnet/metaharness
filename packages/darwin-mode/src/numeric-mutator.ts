...
function toUnit(value: number, spec: { min: number; max: number; scale: 'linear' | 'log' }): number {
  if (spec.scale === 'log') {
...
function fromUnit(unit: number, spec: { min: number; max: number; scale: 'linear' | 'log'; type: 'float' | 'int' }): number {
...
export function defaultGenome(genomeSpec: NumericGenomeSpec): NumericGenome {
...
export function mutateGenome(
  parent: NumericGenome,
  genomeSpec: NumericGenomeSpec,
  seed: number,
  generation: number,
  index: number,
  sigma: number,
): { genome: NumericGenome; mutatedParams: string[] } {
  const rand = mulberry32(hash(seed, generation, index, 'numeric-mutate'));
  const out: Record<string, number> = {};
  const mutatedParams: string[] = [];
  for (const [name, spec] of Object.entries(genomeSpec)) {
    ...
    out[name] = next;
    if (next !== current) mutatedParams.push(name);
  }
...
export function crossoverGenome(
  parentA: NumericGenome,
  parentB: NumericGenome,
  genomeSpec: NumericGenomeSpec,
  seed: number,
  generation: number,
  index: number,
): { genome: NumericGenome; fromB: string[] } {
  const names = Object.keys(genomeSpec);
  const rand = mulberry32(hash(seed, generation, index, 'numeric-crossover'));
  let fromB = names.filter(() => rand() < 0.5);
  if (fromB.length === 0 && names.length > 0) fromB = [names[hash(seed, generation, index) % names.length]];
  if (fromB.length === names.length && names.length > 1) fromB = fromB.slice(0, -1);
  const out: Record<string, number> = { ...parentA };
  for (const name of fromB) {
    if (parentB[name] !== undefined) out[name] = parentB[name];
  }
