...
export const isEmptyPatchDetail = (d: InstanceDetail | null | undefined): boolean => !!d && d.failureClass === 3;

/** Set of instance_ids the run gold-resolved, from an eval JSON's `.details`. */
export function resolvedIdSet(details: EvalDetails = {}): Set<string> {
...
export function emptyPatchRate(details: EvalDetails = {}): number {
...
export function totalThrash(details: EvalDetails = {}): number {
...
export function summarizeEval(ev: EvalArtifact = {}): EvalSummary {
 ...
 */
export function evaluatePromotion({ seed, cand }: { seed: EvalSummary; cand: EvalSummary }): PromotionVerdict {
  ...
  const checks: PromotionChecks = { goldNoRegress, emptyPatchImproves, costPerResolvedNotWorse };
...
export function compositeKey({ host, model, vertical, language, task_class, genome_version }: CompositeKeyParts): string {
...
export interface PromotionRunProvenance {
  ...
  bestMean?: number;
  budget?: unknown;
  holdout?: { goldDelta?: number | null } | null;
  [key: string]: unknown;
}
...
export interface PromotionReportInput {
  ...
  train: { seed: EvalSummary; cand?: EvalSummary | null };
  holdout: { seed: EvalSummary; cand?: EvalSummary | null };
...
}: PromotionReportInput): PromotionReport {
