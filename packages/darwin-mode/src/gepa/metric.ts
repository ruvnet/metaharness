...
export function parseActionRaw(actionRaw: unknown): ParsedAction {
  const s = String(actionRaw ?? '');
  try { const o = JSON.parse(s); if (o && typeof o === 'object') return o as ParsedAction; } catch { /* truncated */ }
...
export interface TranscriptAnalysis {
  steps: number;
  counts: Record<string, number>;   // tool -> count
  runTestsCount: number; runTestsFails: number; runTestsPasses: number;
  editAttempts: number; editsLanded: number; editsFailed: number;
  testFileEditAttempts: number;
  noopCount: number;
  filesRead: string[]; filesEdited: string[];
  repeatedActionWarnings: number;   // the anti-thrash ⚠️ SYSTEM warnings observed
  submitted: boolean; vetoes: number; advisories: number;
}
...
export function analyzeTranscript(transcript: TranscriptEntry[] = []): TranscriptAnalysis {
  ...
  const readSet = new Set<string>(); const editSet = new Set<string>();
  for (const t of transcript) {
    ...
    if (act.tool === 'noop') a.noopCount++;
    if (act.tool === 'read' && act.path) readSet.add(act.path);
    if (act.tool === 'run_tests') {
      a.runTestsCount++;
      if (/ALL TARGET TESTS PASS/.test(obs)) a.runTestsPasses++;
      else if (!/no edits applied yet/.test(obs)) a.runTestsFails++;
    }
    if (act.tool === 'edit' || act.tool === 'line_edit') {
      a.editAttempts++;
      if (/is a test file/.test(obs)) a.testFileEditAttempts++;
      else if (/^(edited |line_edit .*replaced lines)/.test(obs)) { a.editsLanded++; if (act.path) editSet.add(act.path); }
      else a.editsFailed++;
    }
    if (act.tool === 'submit') { if (/VETOED/.test(obs)) a.vetoes++; else a.submitted = true; }
    if (act.tool === 'advise') a.advisories++;
    if (/⚠️ SYSTEM: You already ran this exact action/.test(obs)) a.repeatedActionWarnings++;
  }
  a.filesRead = [...readSet]; a.filesEdited = [...editSet];
  return a;
}
...
const DEFAULT_IS_TEST_PATH: IsTestPath = (p) => /(^|\/)tests?\//.test(p) || /(^|\/)test_[^/]*$|_test\.[a-z]+$/.test(p);

/** Non-test source files touched by a unified diff, + total changed (±) line count. */
export function patchStats(patch: unknown, isTestPath: IsTestPath = DEFAULT_IS_TEST_PATH): PatchStats {
...
export function goldFiles(goldPatch: unknown): string[] { return patchStats(goldPatch).files; }
 ...
 */
export function computeInstanceScore({
  goldResolved = false, resolvedInLoop = false, patch = '', goldPatchFiles = null,
  analysis = analyzeTranscript([]), cost = 0, thrash = 0, costCap = 0.5, isTestPath,
}: ComputeInstanceScoreInput = {}): InstanceScore {
...
export const FAILURE_CLASSES: Record<number, string> = {
  0: 'resolved',
...
export function classifyFailure({
  goldResolved = false, analysis, patch = '', goldPatchFiles = null, maxSteps = 12,
}: ClassifyFailureInput = {}): number {
  const a = analysis || analyzeTranscript([]);
  const ps = patchStats(patch);
  if (goldResolved) return 0;
  if (ps.empty && a.editAttempts === 0) return 3;                                        // pure exploration loop
  if (a.editAttempts > 0 && a.editsLanded === 0) return 2;                               // edits never applied
  const readGold = Array.isArray(goldPatchFiles) && goldPatchFiles.length
    ...
    : null;
  if (readGold === false) return 1;                                                      // never even looked at the fix site
  if (a.runTestsFails >= 3 && a.runTestsPasses === 0) return 5;                          // hammering failing tests
  if (!ps.empty && readGold) return 4;                                                   // right place, wrong fix
  if (a.editsLanded > 0 && a.steps >= maxSteps) return 6;                                // ran out of road
  return ps.empty ? 3 : 4;
}

const MUTATION_HINTS: Record<number, string> = {
...
export function makeFeedback({
  instanceId = '?', analysis, scored, failureClass, goldPatchFiles = null, teacherSummary = null, error = null,
}: MakeFeedbackInput = {}): string {
  if (error) return `${instanceId}: EVALUATION ERROR — ${error}. Scored 0.0 (never-raise contract); no trajectory available.`;
  const a = analysis || analyzeTranscript([]);
  ...
  if (penalties) lines.push(`penalties: ${penalties}.`);
  if (failureClass != null) lines.push(`failure class ${failureClass} (${FAILURE_CLASSES[failureClass]}). ${MUTATION_HINTS[failureClass] || ''}`.trim());
  if (teacherSummary) lines.push(`teacher (paired success/advice): ${String(teacherSummary).slice(0, 700)}`);
  return lines.join('\n');
}
