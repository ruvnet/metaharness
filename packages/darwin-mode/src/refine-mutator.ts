...
const ANNOTATION_VARIANTS: readonly string[] = [
...
export class RefineMutator implements CodeGenerator {
  ...
  async generateMutation(input: {
    ...
    if (this.endpoint) {
      ...
      if ('noop' in remote) return { code: parentCode, summary: remote.noop };
  ...
  private async propose(
    input: { parentCode: string; surface: MutationSurface; parentScore: number; failedTraces: string[] },
    evidence: string[],
    nonce: number,
  ): Promise<{ code: string } | { noop: string }> {
    const sys =
      'You refine ONE file of an AI agent harness with ONE minimal bounded edit — never a rewrite. ' +
    ...
    if (!content) return { noop: `refine: ${this.endpoint} unreachable/malformed response — no-op` };
